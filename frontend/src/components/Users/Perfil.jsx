    ...
    <div className="max-w-lg mx-auto bg-white rounded-xl shadow-lg overflow-hidden my-8 font-sans">
      <div className="p-6 md:p-8">
        ...
        <div className="flex items-center mb-6 pb-4 border-b border-gray-200">
          ...
          <div className="ml-4">
            <h1 className="text-2xl font-bold text-gray-800">{nombre} {apellido}</h1>
            <a href={`mailto:${email}`} className="text-sm text-blue-600 hover:text-blue-800 flex items-center">
        ...
        <InfoSection title="Departamento" icon={<FiBriefcase className="text-indigo-500" />}>
          <InfoItem value={departamento?.nombre || 'No asignado'} />
        </InfoSection>

        {/* Sección de Roles */}
        <InfoSection title="Roles" icon={<FiShield className="text-green-500" />}>
          {roles && roles.length > 0 ? (
            roles.map((rol, index) => (
              <Pill key={`rol-${index}`} text={rol.replace("ROLE_", "")} color="green" />
            ))
          ) : (
        ...
        {/* Sección de Permisos */}
        <InfoSection title="Permisos" icon={<FiKey className="text-yellow-500" />}>
          {permisos && permisos.length > 0 ? (
            permisos.map((permiso, index) => (
              <Pill key={`permiso-${index}`} text={permiso.replace("_", " ")} color="yellow" />
            ))
          ) : (
...
const InfoItem = ({ value, isPlaceholder = false }) => (
  <p className={`text-sm ${isPlaceholder ? 'text-gray-400 italic' : 'text-gray-600'}`}>
    {value}
  </p>
);
...
const Pill = ({ text, color = 'gray' }) => {
  ...
  return (
    <span className={`px-3 py-1 text-xs font-medium rounded-full ${colorClasses[color] || colorClasses.gray}`}>
      {text}
    </span>
  );
};
